...
func (t *twirp) generateUtils() {
	...
	t.P(`  	  respBody = `, t.pkgs["twirp_util"], `.GetOldErrorResp(err)`)
	t.P(`  } else {`)
	t.P(`      respBody = marshalErrorToJSON(twerr)`)
	t.P(`  }`)
	t.P(`  `)
	t.P(`  resp.Header().Set("Content-Type", "application/json; charset=UTF-8") // Error responses are always JSON`)
	...
	t.P(`	  return err`)
	t.P(`	}`)
	t.P(`	glg.Errorf("panic: %v", p)`)
	t.P(`	return errors.New("server exception")`)
	t.P(`}`)
	t.P(``)
...
func (t *twirp) generateServerJSONMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	t.P(`  resp.Header().Set("Content-Type", "application/json; charset=UTF-8")`)
