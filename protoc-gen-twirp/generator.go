...
func (t *twirp) Generate(in *plugin.CodeGeneratorRequest) *plugin.CodeGeneratorResponse {
	...
	t.registerPackageName("twirp_util")
...
func (t *twirp) generateImports(file *descriptor.FileDescriptorProto) {
	...
	t.P(`import `, t.pkgs["twirp_util"], ` "github.com/olachat/gemini/util/twirp_util"`)
...
func (t *twirp) generateUtils() {
	...
	t.P(`  `)
	t.P(`  var respBody []byte`)
	t.P(`  if `, t.pkgs["twirp_util"], `.CheckUseOldFormat(ctx){`)
	t.P(`  	  respBodyStruct := `, t.pkgs["twirp_util"], `.GetOldErrorResp(err)`)
	t.P(`  	  respBody, _ = `, t.pkgs["json"], `.Marshal(respBodyStruct)`)
	t.P(`  } else {`)
	...
	t.P(`	form := `, t.pkgs["url"], `.Values{}`)
	t.P(`	err = encoder.Encode(in, form)`)
	t.P(`	if err != nil {`)
	...
	t.P(`	}`)
	t.P(``)
	t.P(`	req, err := newRequest(ctx, URL, `, t.pkgs["bytes"], `.NewReader([]byte(reqBytes)), "application/x-www-form-urlencoded")`)
	t.P(`	if err != nil {`)
	t.P(`		return ctx, wrapInternal(err, "could not build request")`)
	t.P(`	}`)
	t.P()
...
func (t *twirp) generateService(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto, index int) {
	...
	t.sectionComment(servName + ` FormURLEncoded Client`)
	t.generateClient("FormURLEncoded", file, service)
...
// valid names: 'JSON', 'Protobuf', 'FormURLEncoded'
...
func (t *twirp) generateServerMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	t.P(`    s.serve`, methName, `FormURLEncoded(ctx, resp, req)`)
	t.P(`  default:`)
	...
	t.generateServerFormURLEncodedMethod(service, method)
}
...
func (t *twirp) generateServerJSONMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	t.P(`  if twirp_util.CheckUseOldFormat(ctx) {`)
	t.P(`  	  respContentOldFormat := `, t.pkgs["twirp_util"], `.GetOldSuccessResp(respContent)`)
	t.P(`  	  respBytes, err = `, t.pkgs["json"], `.Marshal(respContentOldFormat)`)
	t.P(`  } else{`)
...
func (t *twirp) generateServerFormURLEncodedMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	methName := methodNameCamelCased(method)
	servName := serviceNameCamelCased(service)
	t.P(`func (s *`, servStruct, `) serve`, methName, `FormURLEncoded(ctx `, t.pkgs["context"], `.Context, resp `, t.pkgs["http"], `.ResponseWriter, req *`, t.pkgs["http"], `.Request) {`)
	t.P(`  var err error`)
	t.P(`  ctx = `, t.pkgs["ctxsetters"], `.WithMethodName(ctx, "`, methName, `")`)
	t.P(`  ctx, err = callRequestRouted(ctx, s.hooks)`)
	t.P(`  if err != nil {`)
	...
	t.P(`decoder.SetAliasTag("json")`)
	t.P(`err = decoder.Decode(reqContent, req.Form)`)
	t.P(`if err != nil {`)
	t.P(`	s.handleRequestBodyError(ctx, resp, "the json request could not be decoded", err)`)
	t.P(`	return`)
	t.P(`}`)
